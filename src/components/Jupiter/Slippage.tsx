...
export const Slippage = ({
  ...
  const [slippageInput, setSlippageInput] = useState<number | undefined>(undefined);
  const [visible, setVisible] = useState(false);

  const customSlippage = !SlippageOptions.includes(slippageInput || -1);

  const canSubmit = !customSlippage || (customSlippage && slippageInput && slippageInput > 0 && slippageInput < 50);

  const handleSave = () => {
    slippageInput && setSlippage(slippageInput);
    setVisible(false);
  };
    ...
    >
      <div>
        ...
        <div className="flex flex-row justify-between mt-5">
          {SlippageOptions.map((e) => {
            return (
              <ButtonBorderGradient
                key={`slippage-option-${e}`}
                onClick={() => setSlippageInput(e)}
                buttonClass="bg-black p-2 uppercase font-bold h-[50px] w-full"
                fromColor={slippageInput === e ? "green-400" : "none"}
                toColor="blue-500"
        ...
        <div className="mt-5">
          <div
            className={clsx(
              "relative",
              customSlippage && "bg-gradient-to-r from-green-400 to-blue-500",
              "p-[2px] rounded-[6px] h-[50px]"
            )}
          >
            <input
              onChange={(e) => setSlippageInput(parseFloat(e.target.value.trim()))}
              placeholder="0.00"
              value={slippageInput === undefined ? '' : slippageInput}
              type="number"
              max={100}
              min={0}
